...
std::shared_ptr<Font> FontMgr::cp437(const std::string &name, std::shared_ptr<Texture> tex, int char_w, int char_h) {
  if (auto it = fonts_.find(name); it != fonts_.end())
    return it->second;

  fonts_[name] = std::make_shared<CP437>(tex, char_w, char_h);
  return fonts_[name];
}
