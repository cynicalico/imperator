...
InputMgr::InputMgr(ModuleMgr &module_mgr) : Module(module_mgr) {
    ...
    event_bus->sub<E_GlfwCharacter>(module_name_, [&](const auto &p) { r_glfw_character_(p); });
    event_bus->sub<E_GlfwCursorPos>(module_name_, [&](const auto &p) { r_glfw_cursor_pos_(p); });
    event_bus->sub<E_GlfwCursorEnter>(module_name_, [&](const auto &p) { r_glfw_cursor_enter_(p); });
    event_bus->sub<E_GlfwMouseButton>(module_name_, [&](const auto &p) { r_glfw_mouse_button_(p); });
    event_bus->sub<E_GlfwScroll>(module_name_, [&](const auto &p) { r_glfw_scroll_(p); });
}
...
void InputMgr::r_update_(const E_Update &p) {
    ...
    event_bus->poll<E_GlfwKey>(module_name_);
    event_bus->poll<E_GlfwCharacter>(module_name_);
    event_bus->poll<E_GlfwCursorPos>(module_name_);
    event_bus->poll<E_GlfwCursorEnter>(module_name_);
    event_bus->poll<E_GlfwMouseButton>(module_name_);
    event_bus->poll<E_GlfwScroll>(module_name_);
