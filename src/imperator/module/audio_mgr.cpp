...
void AudioMgr::unload_sound(Sound &sound) {
    if (const auto it = sources_.find(sound.buffer); it != sources_.end()) {
...
ALuint AudioMgr::load_sound_(const std::filesystem::path &path) {
    ...
    // if(!sndfile)
    // {
    //     fprintf(stderr, "Could not open audio in %s: %s\n", path.string().c_str(), sf_strerror(sndfile));
    //     return 0;
    // }
    // if(sfinfo.frames < 1)
    // {
    //     fprintf(stderr, "Bad sample count in %s (%" PRId64 ")\n", path.string().c_str(), sfinfo.frames);
    //     sf_close(sndfile);
    //     return 0;
    // }
    //
