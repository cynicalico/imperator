...
void register_glfw_error_callback() { glfwSetErrorCallback(internal::error_callback); }
...
void set_glfw_user_pointer(GLFWwindow *window, void *p) { glfwSetWindowUserPointer(window, p); }

void clear_glfw_user_pointer(GLFWwindow *window) { glfwSetWindowUserPointer(window, nullptr); }

namespace internal {
void set_global_callback_user_pointer(void *p) { internal::user_pointer = p; }

void clear_global_callback_user_pointer() { internal::user_pointer = nullptr; }

void error_callback(int code, const char *description) { IMPERATOR_LOG_ERROR("GLFW ({}): {}", code, description); }
