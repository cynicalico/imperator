...
HSL rgba_to_hsl(int r, int g, int b, int a) {
    auto hvcl = rgb_to_hvcl(r, g, b);
    double s = (hvcl[3] == 0.0 || hvcl[3] == 1.0) ? 0.0 : (hvcl[1] - hvcl[3]) / std::min(hvcl[3], 1.0 - hvcl[3]);
...
RGB::RGB(int r, int g, int b, int a)
    : r(internal::clamp(r, 0, 255)), g(internal::clamp(g, 0, 255)), b(internal::clamp(b, 0, 255)),
      a(internal::clamp(a, 0, 255)) {}

RGB rgb(int r, int g, int b) { return {r, g, b, 255}; }
...
RGB rgba(int r, int g, int b, int a) { return {r, g, b, a}; }
...
RGB rgb_f(float r, float g, float b) {
    return {static_cast<int>(r * 255), static_cast<int>(g * 255), static_cast<int>(b * 255), 255};
}
...
RGB RGB::get_inverse() const { return {255 - r, 255 - g, 255 - b, a}; }
...
HSL RGB::to_hsl() const { return internal::rgba_to_hsl(r, g, b, a); }

HSV RGB::to_hsv() const { return internal::rgba_to_hsv(r, g, b, a); }
...
// }
//
// CIELab RGB::to_lab() const { return to_xyz().to_lab(); }
//
// CIELCh RGB::to_lch() const { return to_xyz().to_lch(); }
...
HSV::HSV(double h, double s, double v, int a)
    : h(internal::clamp(h, 0.0, 360.0)), s(internal::clamp(s, 0.0, 1.0)), v(internal::clamp(v, 0.0, 1.0)),
      a(internal::clamp(a, 0, 255)) {}

HSV hsv(double h, double s, double v) { return {h, s, v, 255}; }
...
HSV hsva(double h, double s, double v, int a) { return {h, s, v, a}; }
...
glm::vec4 HSV::gl_color() const { return to_rgb().gl_color(); }

HSV HSV::get_inverse() const { return {std::fmod(h + 180.0, 360.0), s, v, a}; }

void HSV::invert() { h = std::fmod(h + 180.0, 360.0); }
...
RGB HSV::to_rgb() const {
    ...
    return rgba(static_cast<int>(255.0 * rd), static_cast<int>(255.0 * gd), static_cast<int>(255.0 * bd), a);
}
...
//
// CIELab HSV::to_lab() const { return to_rgb().to_lab(); }
//
// CIELCh HSV::to_lch() const { return to_rgb().to_lch(); }
...
HSL::HSL(double h, double s, double l, int a)
    : h(internal::clamp(h, 0.0, 360.0)), s(internal::clamp(s, 0.0, 1.0)), l(internal::clamp(l, 0.0, 1.0)),
      a(internal::clamp(a, 0, 255)) {}

HSL hsl(double h, double s, double l) { return {h, s, l, 255}; }
...
HSL hsla(double h, double s, double l, int a) { return {h, s, l, a}; }
...
glm::vec4 HSL::gl_color() const { return to_rgb().gl_color(); }

HSL HSL::get_inverse() const { return {std::fmod(h + 180.0, 360.0), s, l, a}; }

void HSL::invert() { h = std::fmod(h + 180.0, 360.0); }
...
//
// CIELab HSL::to_lab() const { return to_rgb().to_lab(); }
//
// CIELCh HSL::to_lch() const { return to_rgb().to_lch(); }
//
// /*******
//  * XYZ *
//  *******/
...
//   : x(internal::clamp(x, 0.0, x)),
//     // FIXME: does this have an upper limit?
//     y(internal::clamp(y, 0.0, y)),
//     // FIXME: does this have an upper limit?
//     z(internal::clamp(z, 0.0, z)),
//     // FIXME: does this have an upper limit?
//     a(internal::clamp(a, 0, 255)) {}
//
// XYZ xyz(double x, double y, double z) { return {x, y, z, 255}; }
//
...
// }
//
// XYZ xyza(double x, double y, double z, int a) { return {x, y, z, a}; }
//
...
// }
//
// HSV XYZ::to_hsv() const { return to_rgb().to_hsv(); }
//
// HSL XYZ::to_hsl() const { return to_rgb().to_hsl(); }
//
...
// }
//
// CIELCh XYZ::to_lch() const { return to_lab().to_lch(); }
//
// /**********
//  * CIELab *
//  **********/
...
//
// CIELab lab(double l, double a, double b) { return {l, a, b, 255}; }
//
...
// }
//
// CIELab laba(double l, double a, double b, int alpha) { return {l, a, b, alpha}; }
//
...
// }
//
// RGB CIELab::to_rgb() const { return to_xyz().to_rgb(); }
//
// HSV CIELab::to_hsv() const { return to_rgb().to_hsv(); }
//
// HSL CIELab::to_hsl() const { return to_rgb().to_hsl(); }
//
...
//   : l(internal::clamp(l, 0.0, 100.0)),
//     c(internal::clamp(c, 0.0, c)),
//     // FIXME: does this have an upper limit?
//     h(internal::clamp(h, 0.0, 360.0)),
//     a(internal::clamp(a, 0, 255)) {}
//
// CIELCh lch(double l, double c, double h) { return {l, c, h, 255}; }
//
...
// }
//
// CIELCh lcha(double l, double c, double h, int a) { return {l, c, h, a}; }
//
...
// }
//
// RGB CIELCh::to_rgb() const { return to_lab().to_rgb(); }
//
// HSV CIELCh::to_hsv() const { return to_lab().to_hsv(); }
//
// HSL CIELCh::to_hsl() const { return to_lab().to_hsl(); }
//
// XYZ CIELCh::to_xyz() const { return to_lab().to_xyz(); }
//
