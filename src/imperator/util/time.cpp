...
namespace imp {
std::string timestamp() { return timestamp("{:%Y-%m-%d_%H-%M-%S}"); }

Stopwatch::Stopwatch() { start(); }

void Stopwatch::start() { start_ = time_nsec(); }

void Stopwatch::stop() { end_ = time_nsec(); }

std::uint64_t Stopwatch::elapsed_nsec() const { return end_ - start_; }

double Stopwatch::elapsed_usec() const { return (end_ - start_) / 1e3; }

double Stopwatch::elapsed_msec() const { return (end_ - start_) / 1e6; }
...
double Ticker::dt_usec() { return dt_ / 1e3; }

double Ticker::dt_msec() { return dt_ / 1e6; }

double Ticker::dt_sec() { return dt_ / 1e9; }

std::uint64_t Ticker::elapsed_nsec() const { return last_ - start_; }

double Ticker::elapsed_usec() const { return (last_ - start_) / 1e3; }

double Ticker::elapsed_msec() const { return (last_ - start_) / 1e6; }
