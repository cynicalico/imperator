#include "myco/core/glfw_callbacks.h"
...
void window_close_callback(GLFWwindow *window) {
    EventBus::send_nowait<GlfwWindowClose>(window);
}

void window_size_callback(GLFWwindow *window, int width, int height) {
    EventBus::send_nowait<GlfwWindowSize>(window, width, height);
}

void framebuffer_size_callback(GLFWwindow *window, int width, int height) {
    EventBus::send_nowait<GlfwFramebufferSize>(window, width, height);
}

void window_content_scale_callback(GLFWwindow *window, float xscale, float yscale) {
    EventBus::send_nowait<GlfwWindowContentScale>(window, xscale, yscale);
}

void window_pos_callback(GLFWwindow *window, int xpos, int ypos) {
    EventBus::send_nowait<GlfwWindowPos>(window, xpos, ypos);
}

void window_iconify_callback(GLFWwindow *window, int iconified) {
    EventBus::send_nowait<GlfwWindowIconify>(window, iconified);
}

void window_maximize_callback(GLFWwindow *window, int maximized) {
    EventBus::send_nowait<GlfwWindowMaximize>(window, maximized);
}

void window_focus_callback(GLFWwindow *window, int focused) {
    EventBus::send_nowait<GlfwWindowFocus>(window, focused);
}

void window_refresh_callback(GLFWwindow *window) {
    EventBus::send_nowait<GlfwWindowRefresh>(window);
}

void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
//  if (!ImGui::GetIO().WantCaptureKeyboard)
    EventBus::send_nowait<GlfwKey>(window, key, scancode, action, mods);
}

void character_callback(GLFWwindow *window, unsigned int codepoint) {
//  if (!ImGui::GetIO().WantCaptureKeyboard)
    EventBus::send_nowait<GlfwCharacter>(window, codepoint);
}

void cursor_position_callback(GLFWwindow *window, double xpos, double ypos) {
//  if (!ImGui::GetIO().WantCaptureMouse)
    EventBus::send_nowait<GlfwCursorPos>(window, xpos, ypos);
}

void cursor_enter_callback(GLFWwindow *window, int entered) {
//  if (!ImGui::GetIO().WantCaptureMouse)
    EventBus::send_nowait<GlfwCursorEnter>(window, entered);
}

void mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
//  if (!ImGui::GetIO().WantCaptureMouse)
    EventBus::send_nowait<GlfwMouseButton>(window, button, action, mods);
}

void scroll_callback(GLFWwindow *window, double xoffset, double yoffset) {
//  if (!ImGui::GetIO().WantCaptureMouse)
    EventBus::send_nowait<GlfwScroll>(window, xoffset, yoffset);
}

void joystick_callback(int jid, int event) {
    EventBus::send_nowait<GlfwJoystick>(jid, event);
}

void drop_callback(GLFWwindow *window, int count, const char **paths) {
    EventBus::send_nowait<GlfwDrop>(window, count, paths);
}

void monitor_callback(GLFWmonitor *monitor, int event) {
    EventBus::send_nowait<GlfwMonitor>(monitor, event);
}
