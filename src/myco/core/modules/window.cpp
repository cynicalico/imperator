...
//    EventBus::sub<Update>(name, {ModuleInfo<InputMgr>::name}, [&](const auto &e) { update_(e.dt); });
    EventBus::sub<Update>(name, [&](const auto &e) { update_(e.dt); });

    EventBus::sub<StartFrame>(name, [&](const auto &e) { start_frame_(e); });
    EventBus::sub<EndFrame>(name, {ModuleInfo<Application>::name}, [&](const auto &e) { end_frame_(e); });

    EventBus::sub<GlfwWindowClose>(name, [&](const auto &e) { close_callback_(e); });
...
void Window::end_frame_(const EndFrame &e) {
    ...
    if (should_close())
        EventBus::send_nowait<GlfwWindowClose>(glfw_handle);
}

void Window::close_callback_(const GlfwWindowClose &e) {
    EventBus::send_nowait<Shutdown>();
}
