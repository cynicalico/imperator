...
private:
    ...
    absl::flat_hash_map<std::string, std::size_t> idxs_{};
    absl::flat_hash_map<std::string, std::set<std::string>> saved_deps_{};
    absl::flat_hash_map<std::string, std::set<std::string>> unmet_deps_{};
...
template<typename T>
void PrioListV7<T>::add(const std::string &name, std::set<std::string> deps, T v) {
...
template<typename T>
std::tuple<std::size_t, std::optional<std::size_t>, std::set<std::size_t>>
PrioListV7<T>::find_min_and_max_(const std::string &name, const std::set<std::string> &deps) {
    std::size_t min_idx = 0;
    std::optional<std::size_t> max_idx{};
    std::set<std::size_t> unmet_deps_idxs{};
...
template<typename T>
std::tuple<bool, std::size_t> PrioListV7<T>::bubble_(const std::string &name, std::size_t idx, std::size_t min_idx) {
    while (idx < min_idx) {
...
template<typename T>
std::string PrioListV7<T>::debug_stringify() {
    std::string s = "[";
    std::size_t i = 0;
