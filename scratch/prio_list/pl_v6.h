...
public:
    ...
    PrioListV6(const PrioListV6 &) = delete;
    PrioListV6 &operator=(const PrioListV6 &) = delete;

    PrioListV6(PrioListV6 &&other) noexcept = default;
    PrioListV6 &operator=(PrioListV6 &&other) noexcept = default;
...
private:
    absl::flat_hash_map<std::string, std::size_t> s_to_id_{};
...
template<typename T>
const std::size_t PrioListV6<T>::MAX_SIZE_T = std::numeric_limits<std::size_t>::max();

template<typename T>
void PrioListV6<T>::add(const std::string &name, std::vector<std::string> &deps, T &&v) {
    std::size_t I;
    std::vector<std::size_t> I_deps{};
    hash_get_i_deps_(name, deps, I, I_deps);

    if (idx_[I] != MAX_SIZE_T) {
        MYCO_LOG_WARN("{} already added to PrioListV6 at idx {}", name, idx_[I]);
        return;
    }
...
template<typename T>
void PrioListV6<T>::hash_get_i_deps_(
        const std::string &name,
        std::vector<std::string> &deps,
        std::size_t &I,
        std::vector<std::size_t> &I_deps
...
template<typename T>
void PrioListV6<T>::find_min_and_max_(
        std::size_t I,
        const std::vector<std::size_t> &deps,
        std::size_t &min, std::size_t &max,
        std::vector<std::size_t> &unmet_deps_idxs
...
template<typename T>
bool PrioListV6<T>::adjust_unmet_(std::vector<std::size_t> &unmet_deps_idxs, std::size_t &min_idx) {
...
template<typename T>
std::string PrioListV6<T>::name_from_id(std::size_t id) {
    return id_to_s_[id];
}

template<typename T>
std::string PrioListV6<T>::debug_stringify() const {
    std::string s = "[";
