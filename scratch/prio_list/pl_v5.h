#ifndef MYCO_CORE_PRIO_LIST_H
#define MYCO_CORE_PRIO_LIST_H
...
public:
    ...
    PrioListV5(const PrioListV5 &) = delete;
    PrioListV5 &operator=(const PrioListV5 &) = delete;

    PrioListV5(PrioListV5 &&other) noexcept = default;
    PrioListV5 &operator=(PrioListV5 &&other) noexcept = default;
...
template<typename T>
void PrioListV5<T>::add(const std::string &name, std::vector<std::string> &deps, T &&v) {
    std::size_t I;
    std::vector<std::size_t> I_deps{};
...
template<typename T>
std::string PrioListV5<T>::name_from_id(std::size_t id) {
    return id_to_s_[id];
}
