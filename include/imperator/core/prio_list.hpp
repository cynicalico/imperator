#ifndef IMPERATOR_CORE_PRIO_LIST_HPP
#define IMPERATOR_CORE_PRIO_LIST_HPP

#include "imperator/util/helpers.hpp"
#include <algorithm>
#include <string>
...
bool PrioList<T>::add(const std::string& name, std::vector<std::string>&& deps, T&& v) {
  ...
  for (const auto& d: dep_ids)
    if (std::ranges::contains(pending_dep_lookup_[id], d)) {
...
#endif//IMPERATOR_CORE_PRIO_LIST_HPP
