#ifndef IMPERATOR_GFX_GL_TEXTURE_UNIT_HPP
#define IMPERATOR_GFX_GL_TEXTURE_UNIT_HPP
...
public:
  ...
  TexImage(GfxContext &gfx, const std::filesystem::path &path, bool retro = false);
  TexImage(GfxContext &gfx, TexFormat format, GLsizei w, GLsizei h, bool retro = false);
  ~TexImage();

  TexImage(const TexImage &) = delete;
  TexImage &operator=(const TexImage &) = delete;

  TexImage(TexImage &&other) noexcept;
  TexImage &operator=(TexImage &&other) noexcept;
...
#endif//IMPERATOR_GFX_GL_TEXTURE_UNIT_HPP
